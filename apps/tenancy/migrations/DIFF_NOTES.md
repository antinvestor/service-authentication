...
The snapshot is bootstrap data, not a service catalog. After bootstrap, new
service audiences are accepted as canonical HTTPS **subdomains** of the
platform apex (`OAUTH2_AUDIENCE_BASE_URL=https://stawi.org` →
`https://profile.stawi.org`, `https://tenancy.stawi.org`, …). Permission
namespaces are registered by authenticated services at setup time. Adding a
service does not require an authentication-service source or migration change.
