...
| W16 | Table is still described in comments/migrations as a Timescale hypertable; TimescaleDB was removed platform-wide in #858, the composite PK `(id, created_at)` remains, and no partitioning or retention exists. Pre-v2 rows carry no chain position and are discarded: the v2 chain is greenfield | `migrations/0001/20260420_audit_entries_composite_pk.sql`; commit `83edcb1` |
...
fields: canon_version, tenant_id, partition_id, seq, entry_id, service, manifest_version,
        ...
        relations (deterministic JSON), details (deterministic JSON)
...
1. `audit-signing-key` ExternalSecret projects the existing Vault key as secret key `k1` (rotation adds `k2`…); the HelmRelease mounts the secret as a volume at `/var/run/secrets/audit` (`readOnly: true`, `defaultMode: 0400`), shared with the migration Job, and drops the `AUDIT_SIGNING_KEY` env entry. `AUDIT_SIGNING_KEY_REF=vault://antinvestor/auth/audit/signing#private_key`, `AUDIT_SIGNING_KEY_ID=k1`. Done in `antinvestor/deployments#27`.
...
**Proto publishing.** The additive `audit.v1` changes are on the BSR label `feat-audit-v2` (commit `2d2c6497b768`) so the service and `common/audit` build before merge; CI pushes `main` on merge and the pseudo-versions in `go.mod` are then bumped to the `main` commit.

1. **Schema (setup Job, idempotent):** add columns and tables. **The chain is greenfield:** rows written by the pre-v2 service carry no sequence, key id or canonical hash and are discarded by the migration (`TRUNCATE audit_entries`), so every tenant starts at `seq = 1`. There is no legacy hash path and no backfill.
2. **Key:** the setup Job seeds `audit_signing_keys` with the public half of the key mounted at `/var/run/secrets/audit/k1`.
3. **Deploy the service** with the writer and intake-mode handlers; the direct-chain path is gone. Old `common/audit` clients keep working: `CreateAuditEntry` still accepts the v1 fields, generates `entry_id`, and returns `ACCEPTED`. Because old clients send `profile_id` from claims and the service now applies the actor rule server-side, service-account callers on old clients receive `InvalidArgument` and soft-fail; this is the intended tightening.
...
Rollback: the previous image would write unsequenced rows into the same table and break the chain; roll forward, or freeze the affected tenants and requeue.
...
| Migration | Pre-v2 rows are discarded, unique chain position enforced, re-run is a no-op, triggers hold |
