...
func LoginEndpoint(env *utils.Env, rw http.ResponseWriter, req *http.Request) error {

	span, ctx := opentracing.StartSpanFromContext(req.Context(), "LoginEndpoint")
	defer span.Finish()
	...
	if req.Method == "GET" {
		...
		getLogReq, err := hydra.GetLoginRequest(req.Context(), loginchallenge)
		if err != nil {
			return err
		}
