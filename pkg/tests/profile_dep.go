...
func (d *dependency) Setup(ctx context.Context, ntwk *testcontainers.DockerNetwork) error {
	...
	containerRequest := testcontainers.ContainerRequest{
		...
		Env: map[string]string{
			...
			"OAUTH2_REQUESTED_AUDIENCES":        "https://notification.example.test,https://tenancy.example.test",
			"OAUTH2_RESOURCE_AUDIENCE":          "https://profile.example.test",
			"OAUTH2_SERVICE_AUDIENCE":           "service_notification,service_tenancy",
			"OAUTH2_JWT_VERIFY_AUDIENCE":        "service_profile,https://profile.example.test",
