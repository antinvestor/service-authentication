...
func (d *notificationDependancy) Setup(ctx context.Context, ntwk *testcontainers.DockerNetwork) error {
	...
	if err != nil {
		return fmt.Errorf("failed to fetch JWKS for notification container: %w", err)
	}
	...
	containerRequest := testcontainers.ContainerRequest{
		...
		Env: map[string]string{
			...
			"OAUTH2_SERVICE_CLIENT_ID":          "dev_service_notification",
			"OAUTH2_SERVICE_CLIENT_SECRET":      "hkGiJroO9cDS5eFnuaAV",
			"OAUTH2_TOKEN_ENDPOINT_AUTH_METHOD": "client_secret_post",
			"OAUTH2_AUDIENCE_BASE_URL":          "https://example.test",
			"OAUTH2_REQUESTED_AUDIENCES":        "https://profile.example.test,https://tenancy.example.test,https://devices.example.test",
			"OAUTH2_RESOURCE_AUDIENCE":          "https://notification.example.test",
			"OAUTH2_SERVICE_AUDIENCE":           "service_profile,service_tenancy,service_device",
			"OAUTH2_JWT_VERIFY_AUDIENCE":        "service_notification,https://notification.example.test",
