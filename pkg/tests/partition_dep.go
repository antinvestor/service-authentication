...
func (d *partitionDependancy) Setup(ctx context.Context, ntwk *testcontainers.DockerNetwork) error {
	...
	containerRequest := testcontainers.ContainerRequest{
		...
		Env: map[string]string{
			...
			"OAUTH2_REQUESTED_AUDIENCES":                    "https://notification.example.test,https://profile.example.test,https://authentication.example.test",
			"OAUTH2_RESOURCE_AUDIENCE":                      "https://tenancy.example.test",
