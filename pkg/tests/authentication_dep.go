...
func (d *authenticationDependency) Setup(ctx context.Context, ntwk *testcontainers.DockerNetwork) error {
	...
	containerRequest := testcontainers.ContainerRequest{
		...
		Env: map[string]string{
			...
			"OAUTH2_REQUESTED_AUDIENCES":                    "https://devices.example.test,https://notification.example.test,https://tenancy.example.test,https://profile.example.test",
			"OAUTH2_RESOURCE_AUDIENCE":                      "https://authentication.example.test",
